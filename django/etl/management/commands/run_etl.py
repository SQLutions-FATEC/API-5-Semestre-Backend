    ...
    def handle(self, *args, **kwargs):
        ...
        for extractor_class, loader_fn, model_dw, nome in PIPELINE:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f" -> {nome} sincronizados com sucesso!")
                )
        ...
        try:
            ...
            # 1. Extração
            self.stdout.write(
                " -> Extraindo dados de estoque, solicitações, materiais e pedidos..."
            )
            ...
            df_estoque_transformado = calculate_estoque_saldo(
                df_estoque, df_solicitacoes, df_materiais, df_pedidos
            )
            ...
            validate(
                "Localizações",
                len(df_estoque_transformado['localizacao'].unique()),
                total_loc,
            )
            self.stdout.write(
            ...
            self.stdout.write(
                self.style.ERROR(f" -> Erro ao processar Estoque/Saldo: {e}")
            )
        ...
        if erros:
            ...
            self.stdout.write(
                self.style.ERROR(f"Sincronização finalizada com erros: {erros}")
            )
        else:
