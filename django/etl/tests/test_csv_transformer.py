...
class CsvTransformerTest(TestCase):
    ...
    def test_transformar_csv_projeto(self):
        ...
        self.assertEqual(resultado["custo_hora"][0], 150)

    def test_transformar_csv_tipo_diferente(self):

        df = pd.DataFrame([{"custo_estimado": "150.50"}])

        resultado = transformar_csv(df, "material")

        self.assertEqual(resultado["custo_estimado"][0], 150.50)
