...
def test_calculate_estoque_saldo_logic():
    ...
    df_materiais = pd.DataFrame({'id': [1, 2], 'custo_estimado': [100.0, 50.0]})

    df_solicitacoes = pd.DataFrame(
        {'id': [101, 102], 'material_id': [1, 2], 'quantidade': [10, 5]}
    )
    ...
    df_pedidos = pd.DataFrame(
        {
            ...
            'status': [
                'ENTREGUE',
                'PENDENTE',
            ],  # Apenas material 1 deve passar no filtro final
            'data_pedido': ['2023-01-01', '2023-01-02'],
        }
    )

    # 2. Execução
    result_df = calculate_estoque_saldo(
