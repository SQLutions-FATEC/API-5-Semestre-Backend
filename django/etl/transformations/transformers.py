    ...
    for col in columns:
        if col in df.columns:
            ...
            df[col] = df[col].replace(
                ['NAN', 'NONE', 'NAT', 'NULL', ''], 'NAO INFORMADO'
            )
    return df
...
def calculate_project_metrics(df):
    ...
    df['lead_time_dias'] = (
        (df['data_fim_prevista'] - df['data_inicio']).dt.days.fillna(0).astype(int)
    )
...
def calculate_estoque_saldo(df_estoque, df_solicitacoes, df_materiais, df_pedidos):
    ...
    agg = (
        df_ped_sol.groupby('material_id')
        .agg(
    ...
    df_entregues = df_ped_sol[
        df_ped_sol['status'].str.strip().str.upper() == 'ENTREGUE'
    ]
    if not df_entregues.empty:
        agg_datas = (
            df_entregues.groupby('material_id')
            .agg(data_ultima=('data_pedido', 'max'))
            .reset_index()
        )
    else:
    ...
    df_result = df_estoque.merge(
        df_materiais[['id', 'custo_estimado']],
        left_on='material_id',
        right_on='id',
        suffixes=('', '_mat'),
    )
    ...
    df_result['quantidade_disponivel'] = df_result.apply(
        lambda r: (
            int(r['valor_total_pedidos'] / r['custo_estimado'])
            if r['custo_estimado'] > 0
            else 0
        ),
        axis=1,
    )
    df_result['valor_total'] = df_result['valor_total_pedidos'] - (
        df_result['qtd_solicitada_total'] * df_result['custo_estimado']
    )
