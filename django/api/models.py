...
class DimPrograma(models.Model):
    ...
    data_inicio = ForeignKey(
        DimData, on_delete=CASCADE, related_name='programa_data_inicio'
    )
    data_fim_prevista = ForeignKey(
        DimData, on_delete=CASCADE, related_name='programa_data_fim_prevista'
    )
...
class DimProjeto(models.Model):
    ...
    data_inicio = ForeignKey(
        DimData, on_delete=CASCADE, related_name='projeto_data_inicio'
    )
    data_fim_prevista = ForeignKey(
        DimData, on_delete=CASCADE, related_name='projeto_data_fim_prevista'
    )
    status = CharField(max_length=30)
...
class DimTarefa(models.Model):
    ...
    data_inicio = ForeignKey(
        DimData, on_delete=CASCADE, related_name='tarefa_data_inicio'
    )
    data_fim_prevista = ForeignKey(
        DimData, on_delete=CASCADE, related_name='tarefa_data_fim_prevista'
    )
    status = CharField(max_length=30)
    lead_time_dias = IntegerField(default=0)
    is_atrasado = models.BooleanField(default=False)
...
class FatoCompra(models.Model):
    ...
    data_pedido = ForeignKey(
        DimData, on_delete=CASCADE, related_name='compra_data_pedido'
    )
    data_previsao_entrega = ForeignKey(
        DimData, on_delete=CASCADE, related_name='compra_data_previsao_entrega'
    )
