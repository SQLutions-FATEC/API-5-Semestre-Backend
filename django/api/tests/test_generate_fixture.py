...
class GenerateFixtureCommandTest(TestCase):
    ...
    def test_generate_fixture_success(self, mock_chown, mock_call_command):
        ...
        with patch(
            'api.management.commands.generate_fixture.os.path.dirname'
        ) as mock_dirname:
        ...
        mock_call_command.assert_called_with(
            ...
            'api',
            indent=4,
            stdout=mock_call_command.call_args[1].get('stdout'),
        )
    ...
    @patch(
        'api.management.commands.generate_fixture.os.makedirs',
        side_effect=Exception("Permissão Negada simulada"),
    )
