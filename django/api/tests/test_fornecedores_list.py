...
class ListagemFornecedoresIntegrationTest(TestCase):
    def setUp(self):
        ...
        self.data_mock = DimData.objects.create(dia=24, mes=5, ano=2026)
    ...
    def test_filtro_por_categoria(self):
        ...
        self.assertEqual(
            res_nome.json()[0]['razao_social'], 'RTech Distribuidora 1 Ltda'
        )
    ...
    def test_filtros_combinados(self):
        ...
        response = self.client.get(
            self.url, {'fornecedor_cidade': 'Jundiaí', 'projeto_nome': 'Projeto X'}
        )
        data = response.json()
    ...
    def test_filtros_combinados_sem_resultado(self):
        ...
        response = self.client.get(
            self.url, {'fornecedor_cidade': 'Jundiaí', 'projeto_nome': 'Projeto Y'}
        )
        data = response.json()
