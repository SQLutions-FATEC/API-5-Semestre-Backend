...
class SolicitacoesDetalhesViewTest(TestCase):
    def setUp(self):
        ...
        self.programa = DimPrograma.objects.create(
            codigo_programa="PRG02",
            nome_programa="Prog Detalhes",
            gerente_programa="Gerente D",
            gerente_tecnico="Tecnico D",
            data_inicio=self.data_obj,
            data_fim_prevista=self.data_obj,
            status="Ativo",
        )

        self.projeto = DimProjeto.objects.create(
            codigo_projeto="PRJ03",
            nome_projeto="Proj Detalhes",
        ...
        self.projeto_vazio = DimProjeto.objects.create(
            codigo_projeto="PRJ04",
            nome_projeto="Proj Vazio",
            programa=self.programa,
            responsavel="Resp",
            custo_hora=Decimal('100.00'),
            data_inicio=self.data_obj,
        ...
        self.material = DimMaterial.objects.create(
            ...
            fabricante="Asus",
            custo_estimado=Decimal('150.00'),
            status="Ativo",
        )

        self.fornecedor = DimFornecedor.objects.create(
            codigo_fornecedor="FOR01",
        ...
        self.sol_com_pedido = DimSolicitacao.objects.create(
            numero_solicitacao="S10",
            projeto=self.projeto,
            material=self.material,
            quantidade=2,
            data_solicitacao=self.data_obj,
            prioridade="Normal",
            status="Aprovada",
        )
        FatoCompra.objects.create(
            ...
            status="Enviado",
            solicitacao=self.sol_com_pedido,
            fornecedor=self.fornecedor,
            data_pedido=self.data_obj,
            data_previsao_entrega=self.data_obj,
        )

        self.sol_sem_pedido = DimSolicitacao.objects.create(
            numero_solicitacao="S11",
            projeto=self.projeto,
            material=self.material,
            quantidade=5,
            data_solicitacao=self.data_obj,
    ...
    def test_detalhes_success_with_data(self):
        ...
        response = self.client.get(
            f'/api/projetos/{self.projeto.codigo_projeto}/solicitacoes/detalhes/'
        )
    ...
    def test_detalhes_success_without_data(self):
        ...
        response = self.client.get(
            f'/api/projetos/{self.projeto_vazio.codigo_projeto}/solicitacoes/detalhes/'
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_detalhes_not_found(self):
        ...
        response = self.client.get(
            '/api/projetos/CODIGO-INVALIDO/solicitacoes/detalhes/'
        )
