...
from api.models import DimData, DimPrograma, DimProjeto, DimMaterial, FatoEmpenho
...
class ProjetoEmpenhoViewTest(TestCase):
    def setUp(self):
        ...
        self.programa = DimPrograma.objects.create(
            ...
            data_inicio=self.data_1,
            data_fim_prevista=self.data_2,
            status="Ativo",
        )

        self.projeto_com_dados = DimProjeto.objects.create(
            codigo_projeto="PRJ20",
            nome_projeto="Projeto Empenho",
            programa=self.programa,
            responsavel="Resp Empenho",
        ...
        self.projeto_vazio = DimProjeto.objects.create(
            ...
            nome_projeto="Projeto Sem Empenho",
            programa=self.programa,
            responsavel="Resp Sem Dados",
        ...
        self.material_1 = DimMaterial.objects.create(
            ...
            custo_estimado=Decimal('35.50'),
            status="Ativo",
        )
        self.material_2 = DimMaterial.objects.create(
            codigo_material="MAT02",
            descricao="Tinta",
            categoria="Acabamento",
            fabricante="Suvinil",
            custo_estimado=Decimal('120.00'),
            status="Ativo",
        )
        ...
        FatoEmpenho.objects.create(
            quantidade_empenhada=1000,
            projeto=self.projeto_com_dados,
            material=self.material_3,
            data_empenho=self.data_2,
        )

    def test_empenho_success_with_data(self):
        response = self.client.get(
            f'/api/projetos/{self.projeto_com_dados.codigo_projeto}/empenhos/'
        )
        ...
        categorias = {
            c['categoria']: c['total_custo'] for c in data['empenho_por_categoria']
        }
    ...
    def test_empenho_success_without_data(self):
        response = self.client.get(
            f'/api/projetos/{self.projeto_vazio.codigo_projeto}/empenhos/'
        )
        self.assertEqual(response.status_code, 200)
