...
class SeedDynamicCommandTest(TestCase):
    ...
    def test_seed_creates_basic_objects(self):
        ...
        e roda sem erros.
        """
        call_command('seed_dynamic', programs=2, projects=2, tasks=3, users=2)
    ...
    def test_seed_clear_flag(self):
        ...
        call_command(
            'seed_dynamic', programs=1, projects=1, tasks=1, users=1, clear=True
        )
    ...
    def test_logic_fato_compra_valor_total(self):
        ...
        """
        call_command('seed_dynamic', programs=1, projects=2, tasks=2, users=1)

        compras = FatoCompra.objects.select_related(
            'solicitacao', 'solicitacao__material'
        ).all()
        for compra in compras:
            esperado = round(
                compra.solicitacao.material.custo_estimado
                * compra.solicitacao.quantidade,
                2,
            )
    ...
    def test_logic_fato_empenho_quantity(self):
        ...
        pedidos_entregues = FatoCompra.objects.filter(status='Entregue').select_related(
            'solicitacao__projeto', 'solicitacao__material'
        )
        ...
        for pedido in pedidos_entregues:
            ...
            empenhos_feitos = FatoEmpenho.objects.filter(
                projeto=projeto, material=material
            ).count()
    ...
    def test_logic_fatos_tarefa_hours(self):
        ...
        """
        call_command('seed_dynamic', programs=1, projects=1, tasks=5, users=2)
        ...
        for tarefa in tarefas:
            ...
            if fatos.exists():
                ...
                total_horas = (
                    fatos.aggregate(total=Sum('horas_trabalhadas'))['total'] or 0
                )
    ...
    def test_full_seed_flow(self):
        ...
        """
        call_command('seed_dynamic', programs=2, projects=5, tasks=5, users=3)
