...
class ProjetoTarefasTimesheetViewTest(TestCase):
    def setUp(self):
        ...
        self.programa = DimPrograma.objects.create(
            codigo_programa="PROG02",
            nome_programa="Prog 2",
            gerente_programa="Gerente P2",
            gerente_tecnico="Gerente T2",
        ...
        self.projeto_com_dados = DimProjeto.objects.create(
            ...
            nome_projeto="Projeto Timesheet",
        ...
        self.projeto_vazio = DimProjeto.objects.create(
            ...
            nome_projeto="Projeto Sem Tarefas",
            programa=self.programa,
    ...
    def test_timesheet_success_with_data(self):
        response = self.client.get(
            f'/api/projetos/tarefas/{self.projeto_com_dados.codigo_projeto}'
        )
        self.assertEqual(response.status_code, 200)

        data = response.json()
        self.assertEqual(
            data['projeto']['codigo'], self.projeto_com_dados.codigo_projeto
        )
    ...
    def test_timesheet_success_without_data(self):
        response = self.client.get(
            f'/api/projetos/tarefas/{self.projeto_vazio.codigo_projeto}'
        )
        self.assertEqual(response.status_code, 200)
