...
from api.models import DimData, DimPrograma, DimProjeto, DimMaterial, DimSolicitacao
...
class SolicitacoesStatsViewTest(TestCase):
    def setUp(self):
        ...
        self.data_5d = DimData.objects.create(
            dia=passado_5d.day, mes=passado_5d.month, ano=passado_5d.year
        )
        self.data_2d = DimData.objects.create(
            dia=passado_2d.day, mes=passado_2d.month, ano=passado_2d.year
        )

        self.programa = DimPrograma.objects.create(
            codigo_programa="PROG01",
            nome_programa="Prog Solicitacoes",
            gerente_programa="Gerente S",
            gerente_tecnico="Tecnico S",
            data_inicio=self.data_5d,
            data_fim_prevista=self.data_hoje,
            status="Ativo",
        )

        self.projeto_com_dados = DimProjeto.objects.create(
            codigo_projeto="PRJ01",
            nome_projeto="Projeto com Solicitacoes",
        ...
        self.projeto_vazio = DimProjeto.objects.create(
            codigo_projeto="PRJ02",
            nome_projeto="Projeto Vazio",
            programa=self.programa,
            responsavel="Resp 2",
            custo_hora=Decimal('100.00'),
            data_inicio=self.data_5d,
        ...
        DimSolicitacao.objects.create(
            ...
            quantidade=1,
            data_solicitacao=self.data_5d,
            prioridade="Normal",
            status="Pendente",
        )

        DimSolicitacao.objects.create(
            numero_solicitacao="S02",
            projeto=self.projeto_com_dados,
            material=self.material,
            quantidade=1,
            data_solicitacao=self.data_5d,
        ...
        DimSolicitacao.objects.create(
            ...
            material=self.material,
            quantidade=1,
            data_solicitacao=self.data_2d,
            prioridade="Alta",
            status="Pendente",
        )

        DimSolicitacao.objects.create(
            numero_solicitacao="S04",
            projeto=self.projeto_com_dados,
            material=self.material,
            quantidade=1,
            data_solicitacao=self.data_5d,
    ...
    def test_stats_success_with_data(self):
        ...
        response = self.client.get(
            f'/api/projetos/{self.projeto_com_dados.codigo_projeto}/solicitacoes/stats/'
        )
        ...
        sol_urgente = next(
            s
            for s in estatisticas['urgentes_criticas']
            if s['numero_solicitacao'] == 'S02'
        )
    ...
    def test_stats_success_without_data(self):
        ...
        response = self.client.get(
            f'/api/projetos/{self.projeto_vazio.codigo_projeto}/solicitacoes/stats/'
        )
        self.assertEqual(response.status_code, 200)
