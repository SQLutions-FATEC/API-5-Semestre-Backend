...
class ProjetoAlertasViewTest(TestCase):
    def setUp(self):
        ...
        self.data_hoje = DimData.objects.create(
            dia=hoje.day, mes=hoje.month, ano=hoje.year
        )
        self.data_passado_20d = DimData.objects.create(
            dia=passado_20d.day, mes=passado_20d.month, ano=passado_20d.year
        )
        self.data_passado_25d = DimData.objects.create(
            dia=passado_25d.day, mes=passado_25d.month, ano=passado_25d.year
        )
        self.data_passado_45d = DimData.objects.create(
            dia=passado_45d.day, mes=passado_45d.month, ano=passado_45d.year
        )

        self.programa = DimPrograma.objects.create(
            codigo_programa="PROG03",
            nome_programa="Prog 3",
            gerente_programa="Gerente P3",
            gerente_tecnico="Gerente T3",
            data_inicio=self.data_passado_45d,
            data_fim_prevista=self.data_hoje,
            status="Ativo",
        )

        self.projeto_com_alertas = DimProjeto.objects.create(
            codigo_projeto="PRJ20",
            nome_projeto="Projeto com Alertas",
            programa=self.programa,
            responsavel="Resp Alertas",
        ...
        self.projeto_sem_alertas = DimProjeto.objects.create(
            codigo_projeto="PRJ21",
            nome_projeto="Projeto Sem Alertas",
            programa=self.programa,
            responsavel="Resp Sem Alertas",
            custo_hora=Decimal('100.00'),
            data_inicio=self.data_passado_45d,
            data_fim_prevista=self.data_hoje,
            status="Ativo",
        ...
        self.material_ativo = DimMaterial.objects.create(
            ...
            categoria="Cat",
            fabricante="Fab",
            custo_estimado=Decimal('50.00'),
            status="Ativo",
        )
        self.material_obsoleto = DimMaterial.objects.create(
            codigo_material="M21",
            descricao="Material Obsoleto",
            categoria="Cat",
            fabricante="Fab",
            custo_estimado=Decimal('30.00'),
            status="Obsoleto",
        )

        self.fornecedor = DimFornecedor.objects.create(
            codigo_fornecedor="F20",
        ...
        self.solicitacao_atrasada = DimSolicitacao.objects.create(
            numero_solicitacao="S20",
            projeto=self.projeto_com_alertas,
            material=self.material_ativo,
            quantidade=5,
            data_solicitacao=self.data_passado_45d,
            prioridade="Normal",
            status="Ativo",
        )
        FatoCompra.objects.create(
            ...
            status="Aberto",
            solicitacao=self.solicitacao_atrasada,
            fornecedor=self.fornecedor,
            data_pedido=self.data_passado_45d,
            data_previsao_entrega=self.data_passado_20d,
        )

        self.solicitacao_prioritaria = DimSolicitacao.objects.create(
            numero_solicitacao="S21",
            projeto=self.projeto_com_alertas,
            material=self.material_ativo,
            quantidade=3,
            data_solicitacao=self.data_passado_25d,
            prioridade="Alta",
            status="Aprovada",
        )
        FatoCompra.objects.create(
            ...
            status="Enviado",
            solicitacao=self.solicitacao_prioritaria,
            fornecedor=self.fornecedor,
            data_pedido=self.data_passado_25d,
            data_previsao_entrega=self.data_hoje,
        )

        self.solicitacao_obsoleto = DimSolicitacao.objects.create(
            numero_solicitacao="S22",
            projeto=self.projeto_com_alertas,
            material=self.material_obsoleto,
            quantidade=2,
            data_solicitacao=self.data_passado_20d,
            prioridade="Normal",
            status="Aprovada",
        )
        FatoCompra.objects.create(
            ...
            status="Aberto",
            solicitacao=self.solicitacao_obsoleto,
            fornecedor=self.fornecedor,
            data_pedido=self.data_passado_20d,
            data_previsao_entrega=self.data_hoje,
        )

        self.solicitacao_concluida = DimSolicitacao.objects.create(
            numero_solicitacao="S23",
            projeto=self.projeto_com_alertas,
            material=self.material_ativo,
            quantidade=1,
            data_solicitacao=self.data_passado_45d,
        ...
        FatoCompra.objects.create(
            ...
            status="Concluída",
            solicitacao=self.solicitacao_concluida,
            fornecedor=self.fornecedor,
            data_pedido=self.data_passado_45d,
            data_previsao_entrega=self.data_passado_45d,
        )

        self.solicitacao_recente = DimSolicitacao.objects.create(
            numero_solicitacao="S24",
            projeto=self.projeto_com_alertas,
            material=self.material_ativo,
            quantidade=4,
            data_solicitacao=self.data_hoje,
            prioridade="Urgente",
            status="Aprovada",
        )
        FatoCompra.objects.create(
            ...
            status="Aberto",
            solicitacao=self.solicitacao_recente,
            fornecedor=self.fornecedor,
            data_pedido=self.data_hoje,
            data_previsao_entrega=self.data_hoje,
        )

    def test_alertas_success_with_data(self):
        response = self.client.get(
            f'/api/projetos/criticos/{self.projeto_com_alertas.codigo_projeto}'
        )
        ...
        self.assertEqual(
            alertas['pedidos_prioritarios_pendentes'][0]['numero_pedido'], 'PED21'
        )
        self.assertEqual(
            alertas['pedidos_prioritarios_pendentes'][1]['numero_pedido'], 'PED24'
        )
    ...
    def test_alertas_success_without_data(self):
        response = self.client.get(
            f'/api/projetos/criticos/{self.projeto_sem_alertas.codigo_projeto}'
        )
        self.assertEqual(response.status_code, 200)
