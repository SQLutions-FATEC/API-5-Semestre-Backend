...
def projeto_empenho_api(request, codigo_projeto):
    ...
    custo_por_categoria_qs = (
        empenhos.values(categoria=F('material__categoria'))
        .annotate(total_custo=Sum('custo'))
        .order_by('categoria')
    )
    ...
    empenho_por_material_qs = (
        ...
        .annotate(
            quantidade_total=Sum('quantidade_empenhada'), total_custo=Sum('custo')
        )
        .order_by('descricao')
    )
...
def empenhos_programa(request):
    ...
    return JsonResponse(
        {"resultados": resultados, "valor_total_empenhado": float(total)}
    )
