...
def otimizacao_sobras_api(request, codigo_projeto):
    ...
    sobras_qs = (
        FatoEstoqueSaldo.objects.filter(
            projeto__programa=programa_alvo,
            quantidade_disponivel__gt=0,
            projeto__status__in=['CONCLUIDO', 'CONCLUÍDO', 'SUSPENSO'],
        )
        .exclude(projeto=projeto_alvo)
        .select_related('material', 'projeto', 'localizacao')
    )
    ...
    solicitacoes_abertas = DimSolicitacao.objects.filter(
        projeto=projeto_alvo, status__in=['ABERTO', 'Aberto', 'PENDENTE']
    ...
    estoque_geral_programa = (
        FatoEstoqueSaldo.objects.filter(
            projeto__programa=programa_alvo, quantidade_disponivel__gt=0
        )
        .exclude(projeto=projeto_alvo)
        .values('material__codigo_material')
        .annotate(total_disp=Sum('quantidade_disponivel'))
    )

    disp_geral_dict = {
        item['material__codigo_material']: item['total_disp']
        for item in estoque_geral_programa
    }
