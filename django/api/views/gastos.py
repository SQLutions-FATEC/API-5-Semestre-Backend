...
def detalhamento_gastos_projeto_api(request, codigo_projeto):
    ...
    pedidos_qs = (
        ...
        .select_related('solicitacao__material', 'fornecedor')
        .order_by('-valor_total')
    )

    # calcula o gasto total consolidado
    gasto_total_consolidado = (
        pedidos_qs.aggregate(total=Sum('valor_total'))['total'] or 0.0
    )
    ...
    response_data = {
        "projeto": {"codigo": projeto.codigo_projeto, "nome": projeto.nome_projeto},
