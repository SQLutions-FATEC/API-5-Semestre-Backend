...
def make_mock_compra(numero="PED-001", status="pendente", data_pedido_val=None, data_prev_val=None):
...
class TestFornecedorApi:
    ...
    def test_retorna_dados_corretos_do_fornecedor(self, mock_get_object):
        ...
        # Assert
        mock_get_object.assert_called_once_with(DimFornecedor, codigo_fornecedor="FORN123")
...
class TestFornecedorPedidosApi:
    ...
    @patch("api.views.fornecedores.date") # Mockamos a data atual para garantir o atraso
    @patch("api.views.fornecedores._dim_data_para_date")
    def test_calcula_atrasos_corretamente(self, mock_dim_date, mock_date, mock_filter, mock_get_object, mock_queryset):
        # Arrange
        ...
        mock_queryset.__iter__.return_value = [compra_atrasada, compra_no_prazo, compra_entregue]
        
        # Controla os retornos sucessivos da conversão de datas simuladas
        mock_dim_date.side_effect = [
            date(2024, 5, 1), date(2024, 6, 1),   # Pedido 1 (Pedido, Previsão)
            date(2024, 5, 1), date(2024, 6, 15),  # Pedido 2 (Pedido, Previsão)
            date(2024, 5, 1), date(2024, 6, 1),   # Pedido 3 (Pedido, Previsão)
        ]
    ...
    @patch("api.views.fornecedores.get_object_or_404")
    @patch("api.views.fornecedores.FatoCompra.objects.filter")
    def test_aplica_filtro_por_projeto_quando_parametro_presente(self, mock_filter, mock_get_object, mock_queryset):
        # Arrange
        mock_get_object.return_value = make_mock_fornecedor()
        mock_filter.return_value = mock_queryset
        mock_queryset.__iter__.return_value = [] # Retorna vazio só para passar pelo loop
        
        # Passa o parâmetro id_projeto na requisição
        request = RequestFactory().get('/api/fornecedor/FORN123/pedidos/?id_projeto=PRJ-999')
...
class TestListagemFornecedores:
    ...
    def mock_qs(self):
        ...
        qs.__iter__.return_value = [make_mock_fornecedor(codigo="F001", razao_social="Empresa X")]
        return qs
    ...
    def test_aplica_filtros_simples_sem_chamar_distinct(self, mock_all, mock_qs):
        # Arrange
        mock_all.return_value = mock_qs
        request = RequestFactory().get('/api/fornecedores/?fornecedor_nome=Empresa&fornecedor_cidade=Campinas')
